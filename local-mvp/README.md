...
1. Video processing with YOLOv8 to extract traffic sign detections and pixel coordinates, 
2. Geometric transformation from 2D image coordinates to 3D world distances using camera parameters,
3. GPS coordinate calculation using vehicle position and heading data,
4. Comparison with OSM ground truth to verify accuracy and identify new or missing signs, and
5. Interactive visualization through a Streamlit dashboard that displays detection statistics, temporal analysis, geographic mapping, and audit results.
...
```
Note: items marked by (*) are test input specific, for reference only
...
#### `traffic_sign_detection/vid_input.mp4` (not checked in the repo)
- **Type**: Video File (MP4)
...
#### `traffic_sign_detection/yolov8n.pt` (not checked in the repo)
...
**Key Metrics**: # of total detections → # of estimated unique signs (via clustering) → # verified, # of new, # of missing (with simulated GPS)
